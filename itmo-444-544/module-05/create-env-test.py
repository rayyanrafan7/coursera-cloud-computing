...
# Added this loop to catch all reservations (not just index 0)
allInstances = []
for res in range(0, len(responseEC2['Reservations'])):
    for n in range(0, len(responseEC2['Reservations'][res]['Instances'])):
        allInstances.append(responseEC2['Reservations'][res]['Instances'][n])

if len(allInstances) == correctNumberOfEC2Instances:
    print("Correct number of EC2 instances created, expecting " + str(correctNumberOfEC2Instances) + ", received " + str(len(allInstances)) + ".")
    for instance in allInstances:
        print("InstanceID of: " + instance['InstanceId'])
    grandtotal += 1
else:
    print("Incorrect Number of EC2 instances created, expecting " + str(correctNumberOfEC2Instances) + ", received " + str(len(allInstances)) + ".")
    for instance in allInstances:
        print("InstanceID of: " + instance['InstanceId'])
...
# Added this loop to catch all reservations (not just index 0)
for instance in allInstances:
  print("Checking number of EBS attached to InstanceID of: " + instance['InstanceId'])
  if len(instance['BlockDeviceMappings']) == correctNumberOfEBS:
    print("Correct number of EBS instances attached to InstanceID: " + instance['InstanceId'])
    for j in range(0, len(instance['BlockDeviceMappings'])):
      print("EBS Device name: " + str(instance['BlockDeviceMappings'][j]['DeviceName']))
      print("Volume-ID of: " + str(instance['BlockDeviceMappings'][j]['Ebs']['VolumeId']))
      count += 1
      print("One point...")
  else:
...
# correctNumberOfEBS(2) * correctNumberOfEC2Instances(3) = 6
if count == 6:
  grandtotal+=1
  currentPoints()
